...
import { handleToAddWhiteboardUploadedOfficeNewFile } from '@/components/meet/whiteboard/helpers/utils';
...
export default class HandleRoomMetadata {
          ...
          if (
            ...
            if (!isE2EESupported()) {
              this.that.setErrorStatus(
                this.that.intl('notifications.e2ee-unsupported-browser-title'),
                this.that.intl('notifications.e2ee-unsupported-browser-msg')
              );
            } else {
  ...
  private showRecordingNotification = () => {
    ...
    if (!isActiveRecording && this.metadata?.is_recording) {
      toast(this.that.intl('room-metadata.session-recording'), {
        type: 'info',
      });
    } else if (isActiveRecording && !this.metadata?.is_recording) {
      toast(this.that.intl('room-metadata.session-not-recording'), {
        type: 'info',
      });
    }
  };
  ...
  private showRTMPNotification = () => {
    ...
    if (!isActiveRtmpBroadcasting && this.metadata?.is_active_rtmp) {
      toast(this.that.intl('room-metadata.rtmp-started'), {
        type: 'info',
      });
    } else if (isActiveRtmpBroadcasting && !this.metadata?.is_active_rtmp) {
      toast(this.that.intl('room-metadata.rtmp-stopped'), {
        type: 'info',
      });
    }
  };
