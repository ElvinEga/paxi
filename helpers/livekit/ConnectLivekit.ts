...
import type { IErrorPageProps } from "@/components/meet/extra-pages/Error";
import {
...
import type { LivekitInfo } from "./hooks/useLivekitConnect";
	...
	implements IConnectLivekit
{
	...
	private onDisconnected = (reason?: DisconnectReason) => {
		this.errorState({
			title: this.intl("notifications.room-disconnected-title"),
	...
	private getDisconnectErrorReasonText = (reason?: DisconnectReason) => {
		let msg = this.intl("notifications.room-disconnected-unknown", {
			code: "UNKNOWN_REASON",
		});

		switch (reason) {
			case DisconnectReason.CLIENT_INITIATED:
				msg = this.intl("notifications.room-disconnected-client-initiated", {
					code: "CLIENT_INITIATED",
				});
				break;
			case DisconnectReason.DUPLICATE_IDENTITY:
				msg = this.intl("notifications.room-disconnected-duplicate-entry", {
					code: "DUPLICATE_IDENTITY",
				});
				break;
			case DisconnectReason.SERVER_SHUTDOWN:
				msg = this.intl("notifications.room-disconnected-server-shutdown", {
					code: "SERVER_SHUTDOWN",
				});
				break;
			case DisconnectReason.PARTICIPANT_REMOVED:
				msg = this.intl("notifications.room-disconnected-participant-removed", {
					code: "PARTICIPANT_REMOVED",
				});
				break;
			case DisconnectReason.ROOM_DELETED:
				msg = this.intl("notifications.room-disconnected-room-ended", {
					code: "ROOM_ENDED",
				});
				break;
			case DisconnectReason.STATE_MISMATCH:
				msg = this.intl("notifications.room-disconnected-state-mismatch", {
					code: "STATE_MISMATCH",
				});
				break;
		}
